...
### 4.9 自動テスト方針（要件確定・未実装）

自動テストは一切整備されておらず、動作確認は毎回手動で行っている（[既知の制約](#7-既知の制約)）。2026-08-24に方針確定。`test-strategy`スキルの単体/結合/GUIの3層構成に沿う。実装は別途着手する。

| # | テストレベル | 方針 |
|---|---|---|
| 1 | 単体テスト | 純粋ロジックを優先してカバーする。`ClaudeRunEngine.BuildPrompt`（プレースホルダ置換）、`ArtifactService.ResolveWithinRoot`（パス検証・トラバーサル対策）、および4.3〜4.8で新たに決めたロジック（排他判定、ループ回数カウント、保持件数プルーニング、孤児判定＋安全弁）など、間違えると影響が大きい箇所を優先する |
| 2 | 結合テスト | 主要エンドポイント（Run開始、SSE配信、4.3〜4.8で決めた新機能に関わるもの等）を対象に含める。下位レイヤ（`ClaudeRunEngine`・`ArtifactService`・`JsonFileStore`等）をモックに置き換えず、実際の呼び出し関係のまま検証し、関数・モジュール間の連携不具合を検出することを目的とする |
| 3 | GUI/E2E | 自動テストとしては整備しない。ただし正常系の動作確認はAIエージェントによるスクリーンショット確認を許容する（本ドキュメント4.2の視認性確認で実施した方式）。異常系の検証やクリック操作の網羅的自動化は行わない。意図しない挙動が発生した場合は連続実行せずその場で中断し、原因究明は静的な方法（コード読解等）に切り替える |
| 4 | テストプロジェクト | `src/ClaudeCodeGui.Tests`等、xUnitベースの新規プロジェクトを追加する必要がある（現状皆無） |
