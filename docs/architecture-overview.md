...
上記4項目への対応方針は以下の通り確定（2026-08-24）。実装は別途着手する。

| # | 箇所 | 決定内容 |
|---|---|---|
| 1 | Issue詳細画面全体 | 現状維持（タブ切り替え等の構造変更は行わない）。プロトタイプ段階として許容し、他の優先事項を優先する |
| 2 | 実行ログビュー | 「まだ実行していません」等のプレースホルダテキストを追加する |
| 3 | 工程実行の操作列 | 対応不要（現状は実害なし。テンプレート名が長くなる想定がなければ様子見） |
| 4 | 成果物ブラウザ | ツリーとエディタの高さをCSS変数で揃える |
...
### 4.4 SSE再接続方針（要件確定・未実装）

[既知の制約](#7-既知の制約)にある「SSEの自動再接続なし」を解消するための方針。2026-08-24に要件確定。実装は別途着手する。

| # | 論点 | 決定内容 |
|---|---|---|
| 1 | 実装方針 | 「runIdを指定してログ表示をやり直す」共通の再接続関数として実装する。ページ再読込時の再接続と、接続断からの自動復帰を同じ関数で扱う統合設計とする（別々の機能として作らない） |
| 2 | 再接続時のログ表示 | 再接続前に画面のログ表示を一旦クリアしてから、`StreamLogAsync`が返す既存ログ全体を再描画する（`StreamLogAsync`は毎回ログファイルの内容を最初から返すため、クリアしないと二重表示になる） |
| 3 | 接続断からの自動復帰のトリガー | `app.js`の`EventSource`の`onerror`発生時に、上記の再接続関数を呼び出す（多少の遅延を入れて再試行し、`es.close()`で即座に諦めない） |
| 4 | 運用前提 | ブラウザを閉じてもRunは`ClaudeRunEngine`（サーバープロセス側）でバックグラウンド実行を継続する前提で開発を進める（`ClaudeRunEngine.StartAsync`が`Task.Run`でHTTPリクエストから切り離して実行しているため、既にこの前提で動く設計になっている）。ページ再読込・別タブでの再開を正式にサポートする |

> 関連する既知の制約: サーバープロセス自体（`dotnet run`）が停止した場合はこの限りでない。実行中Runの状態はメモリ上(`_active`)にしかなく、プロセスが落ちると`Run.Status`が`"running"`のまま更新されずに孤児化する。これを検知・復旧する仕組みは本項の対象外（別途検討が必要な場合は追加の論点とする）。

### 4.5 同時実行の排他制御（要件確定・未実装）

[既知の制約](#7-既知の制約)にある「同時実行の排他制御なし」を解消するための方針。2026-08-24に要件確定。実装は別途着手する。

| # | 論点 | 決定内容 |
|---|---|---|
| 1 | 排他方針 | 同一Issueに対しては同時に1つのRunのみ許可する。キュー化はせず、実行中に新規Run開始を試みた場合は拒否する |
| 2 | 判定方法 | `RunContext`（`ClaudeRunEngine`のprivate nested class）に`IssueId`を追加し、`StartAsync`開始時に`_active.Values`から同一IssueIdの実行中Runがないか確認する |
| 3 | 拒否時の挙動 | `Run.Status = "failed"`とし、既存の実行中RunIdをレスポンスに含める（既存の実行中Runには影響しない） |
| 4 | 実行中Runの停止手段 | 既存の`CancelAsync`/`POST /api/runs/{id}/cancel`（`ClaudeRunEngine.cs:164-184`）をそのまま使う。フロント側の中止ボタンは現状JS内ローカル変数`currentRunId`に依存しページ再読込後は使えなくなるため、4.4で実装する「Issueの実行中Runを検出する」仕組みを再利用し、ページ再読込後や上記3の拒否時にも中止操作へ誘導できるようにする |
| 5 | 排他拒否時のUX | 新規実行が拒否された際、フロント側は返された既存RunIdを使って「このIssueは実行中です。中止しますか？」等の形でその場から中止操作に誘導する |

> 論点4・5は4.4（SSE再接続）で実装する「実行中Run検出」の仕組みをそのまま再利用する設計とする（トピックの近さではなく実際の技術依存）。

**将来の自律ループモード（次のステップ）との関係**: 「同一Issueは同時に1Runまで」という制約は、ループモードが「前工程の完了を待ってから次工程を自動起動する」という動作の技術的な前提になる。

### 4.6 自律ループモード（要件確定・未実装）

README「次のステップ」にある自律ループモードの要件。2026-08-24に要件確定。実装は別途着手する。

**スコープ**: 「工程のRunが成功したら次工程を自動的に起動する」シンプルなシーケンサーとする。`waterfall-dev-workflow`スキルの自律ループモード章（レビュー再判定・非収束時のエスカレーション判断を含む本格的な自動化）とは別物であり、それらは対象外。

| # | 論点 | 決定内容 |
|---|---|---|
| 1 | 有効化単位 | Issueごとに明示的にON/OFF（例: `Issue.LoopEnabled`）。全Issue共通のグローバル設定にはしない（意図せず全件が自動進行するのを避ける） |
| 2 | 次工程のテンプレート選択 | アプリ全体でStageごとに既定テンプレートを1つ設定できるようにする（基本方針）。将来Issueごとに上書きできるよう拡張する余地を残した設計にする（今回はIssueごとの上書きは実装しない） |
| 3 | 成功後の遷移 | Run成功 → `Issue.CurrentStage`を次工程に更新 → その工程の既定テンプレートで次のRunを自動起動。最終工程(`deployment`)が成功したら`Issue.Status = "done"`にしてループを終了する |
| 4 | 失敗時の挙動 | 即座に停止し、`Issue.LoopEnabled`をfalseに戻す。Issue一覧・詳細画面で「ループ停止中（要確認）」等が分かる表示にする |
| 5 | パーミッションモード | ループ実行中は人間が都度選べないため、Issueごとに既定のパーミッションモードを設定できるようにする |
| 6 | 開始・停止操作 | 「自律ループ開始」を既存の「実行」ボタンとは独立した操作にする。停止は既存の「中止」ボタン（4.5）を流用しつつ、中止時は現在のRunだけでなく`Issue.LoopEnabled`もfalseに戻す |
| 7 | 連続自動実行回数の上限 | 4回。5工程を最後まで自動で通すのに必要な最小回数（工程間の遷移は要件定義→設計→実装→テスト→デプロイの4回）に合わせる。失敗がなくても4回遷移した時点で自動停止し、人間の再確認を要する |
| 8 | 手動実行との区別 | ループ由来のRunにのみ自動遷移をトリガーする（`Run`に区別用フラグ、例: `TriggeredByLoop`を追加）。`Issue.LoopEnabled = true`中でも、通常の「実行」ボタンからの手動実行は単発として扱い、自動遷移のトリガーにしない |

> 将来の拡張候補（今回は対象外）: Issueごとのテンプレート上書き（論点2）、レビュー再判定・エスカレーション判断（`waterfall-dev-workflow`スキル相当の本格運用）。

### 4.7 Run/ログの保持方針（要件確定・未実装）

`run-logs/{runId}.log`（`ClaudeRunEngine`）と`runtime-data/runs/{runId}.json`（`JsonFileStore<Run>`）はいずれも削除・ローテーション処理を持たず無制限に蓄積する。自律ループモード（4.6）が入ると1回の自動進行で最大4件のRunが増えるため蓄積が加速する。2026-08-24に方針確定。実装は別途着手する。

| # | 論点 | 決定内容 |
|---|---|---|
| 1 | 保持方針 | 件数ベース。Issueごとに直近N件のRun（およびそれに対応するログファイル）のみ残し、それより古いものを削除する |
| 2 | 保持件数N | 20件 |

- 削除トリガーのタイミング（新規Run作成時に都度チェックするか、起動時にまとめて行うか等）は未確定・次回の設計工程で詰める

### 4.8 Issue削除時に孤児化するRun/ログの扱い（要件確定・未実装）

`DELETE /api/issues/{id}`（`Program.cs:63-67`）はIssue自体のみを削除し、関連する`Run`・ログファイルは削除されず孤児化する（[既知の制約](#7-既知の制約)）。加えて、`runtime-data/issues/`配下のJSONファイルを直接削除するなど、GUI/APIを経由しない削除でも孤児化しうる。2026-08-24に方針確定。実装は別途着手する。

**方式**: イベント駆動のカスケード削除ではなく、状態を見て判断する「孤児掃除」方式を採る（`runtime-data/runs/*.json`を走査し、参照先の`IssueId`が`runtime-data/issues/`に存在しないRunを検出する）。GUI/API経由・ファイル直接操作どちらの削除でも孤児化を検出できる。

> **安全上の注意**: 「Issueが見つからない＝削除された」という判定は、Issueストアの読み取りが一時的に失敗・空振りした場合（ディスクI/O異常、アンチウイルスによる一時ロック、権限問題、コードのバグ等）に、全Issueが削除されたと誤判定して全Runを一括削除してしまうリスクがある。以下の安全策込みで実装する。

| # | 論点 | 決定内容 |
|---|---|---|
| 1 | 削除方法 | 即座に完全削除ではなく、`runtime-data/orphaned/`等へ退避（quarantine）する。復元可能な状態を保ち、一定期間後に手動またはバッチで完全削除する |
| 2 | 誤判定への安全弁 | Issueストアの一覧取得が失敗、または「孤児と判定されるRunの割合が異常に高い（例: 50%超）」場合は、処理を中断してログに警告を出すだけにする（削除・退避を実行しない） |
| 3 | 監査ログ | どのRunIdをどの理由（対応Issue不在）で退避したか、ログに記録する |

- 実行タイミング（アプリ起動時／手動トリガー等）は未確定・次回の設計工程で詰める
