...
| CON-03 | 制約 | 工程実行の操作列（テンプレート選択・パーミッションモード選択・実行/中止ボタン）は対応不要とする（現状実害なしのため） |
...
- **2.4節（SSE再接続）と2.5節（排他制御）の関係**: CON-06の通り、2.5節の拒否時UX（REQ-13）とページ再読込後の中止操作は、2.4節で実装する「実行中Run検出」の仕組みを再利用する設計であり、architecture-overview.mdの記載（トピックの近さではなく実際の技術依存）と一致する。2.4節の実装が2.5節の一部機能（REQ-13の中止誘導）の前提になる。
...
|---|---|---|
| SSEの自動再接続なし | 解消対象 | REQ-07〜REQ-09、CON-05（2.4節） |
